...
public class ToolsController {
    ...
    public String tools(@RequestParam(value = "host", required = false) String host, @RequestParam(value = "fortunefile", required = false, defaultValue = "literature") String fortuneFile, Model model) {
        if (host != null && !host.isBlank()) {
            model.addAttribute("ping", ping(host));
        }
    ...
    private String ping(String host) {
        ...
        try {
            proc = new ProcessBuilder("ping", "-c", "1", host).start();
    ...
    private String fortune(String fortuneFile) {
        ...
        Process proc;
        try {
            proc = new ProcessBuilder("sh", "-c", cmd).start();
            proc.waitFor(5, TimeUnit.SECONDS);
            InputStreamReader isr = new InputStreamReader(proc.getInputStream());
            BufferedReader br = new BufferedReader(isr);
            String line;
